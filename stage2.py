...
def add_stock_route(portfolio_id):
    portfolio = UserPortfolio.query.get_or_404(portfolio_id)
    if request.method == 'POST':
        ticker_input = request.form.get('ticker_symbol', '').strip()
        shares_input = request.form.get('shares', '').strip()
        if not ticker_input:
            return f"<p>Error: Please provide a ticker symbol. <a href='/portfolios/{portfolio_id}/add_stock'>Try again</a></p>"
        ticker = ticker_input.upper()
        try:
            stock_data = yf.Ticker(ticker).info
            company_from_api = stock_data.get('shortName')
            price = stock_data.get('regularMarketPrice', None)
            if not company_from_api or price is None:
                return f"<p>Error: Ticker {ticker} not found or price unavailable. <a href='/portfolios/{portfolio_id}/add_stock'>Try again</a></p>"
            company = company_from_api
        except Exception as e:
            return f"<p>Error accessing Yahoo Finance API: {str(e)}. <a href='/portfolios/{portfolio_id}/add_stock'>Try again</a></p>"
        stock = Stock.query.filter_by(ticker_symbol=ticker).first()
        if not stock:
            stock = Stock(ticker_symbol=ticker, company_name=company, price=price)
        ...
        try:
            shares = int(shares_input) if shares_input else 1
            if shares < 1:
                shares = 1
        except ValueError:
            shares = 1
        
        existing_entry = PortfolioEntry.query.filter_by(portfolio_id=portfolio_id, stock_id=stock.stock_id).first()
        if not existing_entry:
            new_entry = PortfolioEntry(portfolio_id=portfolio_id, stock_id=stock.stock_id, shares=shares)
        ...
        <body>
            ...
            <form method="POST">
                <label>Ticker Symbol (exact match):</label><br>
                <input type="text" name="ticker_symbol" placeholder="e.g., AAPL or INTC" /><br><br>
                <label>Number of Shares:</label><br>
                <input type="number" name="shares" value="1" min="1" /><br><br>
                <button type="submit">Add Stock</button>
            </form>
            <p><a href="/portfolios/{portfolio.portfolio_id}/detail">Cancel</a></p>
        </body>
        </html>
        """
